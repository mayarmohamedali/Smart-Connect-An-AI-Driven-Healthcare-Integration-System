...
class InsuranceForecast {
    ...
    public function getSavedForecast(string $company): array {
        $url = $this->base_url . "/api/insurance/forecast?company=" 
               . urlencode($company);

        // curl_init can return false if cURL is misconfigured
        $ch = curl_init($url);
        if ($ch === false) {
            return ["status" => "error", "error" => "curl_init failed"];
        }
        ...
        // cURL transport error (Flask not running, port wrong, etc.)
        if ($body === false || $errNo !== 0) {
            return ["status" => "error", "error" => "cURL error $errNo: $errMsg"];
        }

        // Fix Python's NaN (not valid JSON) before decoding
        ...
        if (json_last_error() !== JSON_ERROR_NONE) {
            return [
                ...
                "raw"    => substr($body, 0, 300), // helps debug
            ];
        }
