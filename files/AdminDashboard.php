...
$q2 = $conn->query("SELECT COUNT(*) AS total FROM hospitals");
if ($q2) {
    $row = $q2->fetch_assoc();
    $totalHospitals = (int)$row["total"];
}

$q3 = $conn->query("SELECT COUNT(*) AS total FROM medical_insurances");
if ($q3) {
    $row = $q3->fetch_assoc();
    $totalInsurances = (int)$row["total"];
}
...
$q5 = $conn->query("SELECT COUNT(*) AS total FROM users WHERE is_active = 1");
if ($q5) {
    $row = $q5->fetch_assoc();
    $activeUsers = (int)$row["total"];
}

$q6 = $conn->query("SELECT COUNT(*) AS total FROM medical_insurances WHERE policy_completed = 1");
if ($q6) {
    $row = $q6->fetch_assoc();
    $completedPolicies = (int)$row["total"];
}

$q7 = $conn->query("SELECT COUNT(*) AS total FROM medical_insurances WHERE policy_completed = 0");
if ($q7) {
    $row = $q7->fetch_assoc();
    $pendingPolicies = (int)$row["total"];
}
...
$sqlRecentPatients = "
    ...
    LIMIT 5
";
$resRecentPatients = $conn->query($sqlRecentPatients);
if ($resRecentPatients) {
    while ($p = $resRecentPatients->fetch_assoc()) {
        $recentPatients[] = $p;
    }
}
...
$sqlRecentRecords = "
    ...
    LIMIT 5
";
$resRecentRecords = $conn->query($sqlRecentRecords);
if ($resRecentRecords) {
    while ($r = $resRecentRecords->fetch_assoc()) {
        $recentRecords[] = $r;
    }
}

/* =========================
   Latest patient for report
========================= */
$latestPatientName = "N/A";
if (!empty($recentPatients)) {
    $latestPatientName = $recentPatients[0]["full_name"];
}
