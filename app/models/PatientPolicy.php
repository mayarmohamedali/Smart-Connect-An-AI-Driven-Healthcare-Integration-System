<?php
/**
 * PatientPolicy Class - CORRECTED
 * ✅ FIXED: Changed 'id' to 'patient_policy_id' (correct column name)
 */
class PatientPolicy {
    private $conn;
    ...
    // Getters
    public function getPatientPolicyId() { return $this->patient_policy_id; }
    ...
    /**
     * Load patient policy by patient_id
     * ✅ FIXED: Changed pp.id to pp.patient_policy_id
     */
    public function loadByPatientId($patient_id) {
        $stmt = $this->conn->prepare("
            SELECT pp.patient_policy_id, pp.patient_id, pp.insurance_id, 
                   pp.insurance_plan_id, pp.policy_number, pp.start_date, 
                   pp.end_date, pp.status, pp.created_at,
                   ip.plan_name
    ...
    public function getPoliciesByPatient($patient_id) {
        $stmt = $this->conn->prepare("
            SELECT pp.patient_policy_id, pp.policy_number, pp.start_date, 
                   pp.end_date, pp.status, pp.created_at,
                   ip.plan_name,
                   mi.name AS insurance_name
            FROM patient_policy pp
            LEFT JOIN insurance_plan ip ON ip.id = pp.insurance_plan_id
            LEFT JOIN medical_insurances mi ON mi.insurance_id = pp.insurance_id
            WHERE pp.patient_id = ?
            ORDER BY pp.patient_policy_id DESC
        ");
        $stmt->bind_param("i", $patient_id);
