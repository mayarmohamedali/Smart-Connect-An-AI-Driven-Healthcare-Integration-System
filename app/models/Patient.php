<?php

class Patient {
    private $conn;
    private $patient_id;
    private $full_name;
    private $national_id;
    private $phone;
    private $gender;
    private $address;
    private $insurance_id;
    private $is_active;
    private $insurance_name;
    
    public function __construct($conn) {
        $this->conn = $conn;
    }
    
    // Getters
    public function getPatientId() { return $this->patient_id; }
    public function getFullName() { return $this->full_name; }
    public function getNationalId() { return $this->national_id; }
    public function getPhone() { return $this->phone; }
    public function getGender() { return $this->gender; }
    public function getAddress() { return $this->address; }
    public function getInsuranceId() { return $this->insurance_id; }
    public function getIsActive() { return $this->is_active; }

    public function getInsuranceName() { 
    return $this->insurance_name; 
}

    
    // Setters
    public function setFullName($name) { $this->full_name = $name; }
    public function setNationalId($id) { $this->national_id = $id; }
    public function setPhone($phone) { $this->phone = $phone; }
    public function setGender($gender) { $this->gender = $gender; }
    public function setAddress($address) { $this->address = $address; }
    public function setInsuranceId($id) { $this->insurance_id = $id; }
    
    public function loadById($patient_id) {
    $stmt = $this->conn->prepare("
        SELECT 
          p.patient_id, p.full_name, p.national_id, p.phone, p.gender, p.address, 
          p.insurance_id, p.is_active, p.created_at,
          mi.name AS insurance_name
        FROM patients p
        LEFT JOIN medical_insurances mi ON mi.insurance_id = p.insurance_id
        WHERE p.patient_id = ?
        LIMIT 1
    ");
    $stmt->bind_param("i", $patient_id);
    $stmt->execute();
    $row = $stmt->get_result()->fetch_assoc();
    $stmt->close();

    if ($row) {
        $this->patient_id    = $row["patient_id"];
        $this->full_name     = $row["full_name"];
        $this->national_id   = $row["national_id"];
        $this->phone         = $row["phone"];
        $this->gender        = $row["gender"];
        $this->address       = $row["address"];
        $this->insurance_id  = $row["insurance_id"];
        $this->is_active     = $row["is_active"];

        // ✅ IMPORTANT
        $this->insurance_name = $row["insurance_name"] ?? null;

        return true;
    }
    return false;
}

    
    /**
     * Create new patient
     * ✅ FIXED: Removed added_by_hospital_id parameter (column doesn't exist)
     */
public function create($hospital_id = null, $insurance_id = null) {
    $stmt = $this->conn->prepare("
        INSERT INTO patients (full_name, national_id, phone, gender, address, 
                             insurance_id, is_active)
        VALUES (?, ?, ?, ?, ?, ?, 1)
    ");

    // ✅ FIXED: correct bind_param types
    $stmt->bind_param("sssssi", 
        $this->full_name, 
        $this->national_id, 
        $this->phone, 
        $this->gender, 
        $this->address, 
        $insurance_id
    );
    
    if ($stmt->execute()) {
        $this->patient_id = $stmt->insert_id;
        $stmt->close();
        return true;
    }
    $stmt->close();
    return false;
}

    
    /**
     * Search patients - works for both hospital and insurance staff
     * ✅ FIXED: Removed added_by_hospital_id filter (column doesn't exist)
     * 
     * @param string $search_query Search term
     * @param int|null $hospital_id Not used anymore (kept for backward compatibility)
     * @return array List of patients
     */
    public function searchPatients($search_query, $hospital_id = null) {
        // If empty search, return recent patients
        if ($search_query === "") {
            $stmt = $this->conn->prepare("
                SELECT patient_id, full_name, national_id, phone, gender, address, is_active
                FROM patients
                WHERE is_active = 1
                ORDER BY patient_id DESC
                LIMIT 50
            ");
            $stmt->execute();
        } else {
            // Search by name, national_id, or phone
            $like = "%" . $search_query . "%";
            $stmt = $this->conn->prepare("
                SELECT patient_id, full_name, national_id, phone, gender, address, is_active
                FROM patients
                WHERE (full_name LIKE ? OR national_id LIKE ? OR phone LIKE ?)
                  AND is_active = 1
                ORDER BY patient_id DESC
                LIMIT 50
            ");
            $stmt->bind_param("sss", $like, $like, $like);
            $stmt->execute();
        }
        
        $result = $stmt->get_result();
        $patients = [];
        while ($row = $result->fetch_assoc()) {
            $patients[] = $row;
        }
        $stmt->close();
        return $patients;
    }
    
    /**
     * Get patients by hospital - using insurance contracts
     * ✅ NEW: Proper way to get hospital's patients via insurance relationships
     */
public function getPatientsByHospital($hospital_id, $search = "") {
    $sql = "
        SELECT DISTINCT
            p.patient_id,
            p.full_name,
            p.national_id,
            p.phone,
            p.gender,
            p.address,
            p.is_active,
            p.created_at,
            p.insurance_id,
            mi.name AS insurance_name
        FROM patients p
        LEFT JOIN medical_insurances mi
               ON mi.insurance_id = p.insurance_id
        LEFT JOIN insurance_hospitals ih
               ON ih.insurance_id = p.insurance_id
        WHERE p.is_active = 1
          AND (ih.hospital_id = ? OR ? = 0)
    ";

    $types  = "ii";
    $params = [(int)$hospital_id, (int)$hospital_id];

    if ($search !== "") {
        $sql .= " AND (
                    p.full_name LIKE CONCAT('%', ?, '%')
                 OR p.national_id LIKE CONCAT('%', ?, '%')
                 OR p.phone LIKE CONCAT('%', ?, '%')
                )";
        $types .= "sss";
        $params[] = $search;
        $params[] = $search;
        $params[] = $search;
    }

    $sql .= " ORDER BY p.patient_id DESC LIMIT 50";

    $stmt = $this->conn->prepare($sql);
    $stmt->bind_param($types, ...$params);
    $stmt->execute();
    $res = $stmt->get_result();

    $out = [];
    while ($row = $res->fetch_assoc()) $out[] = $row;

    $stmt->close();
    return $out;
}

    
    /**
     * Get patients by insurance
     */
    public function getPatientsByInsurance($insurance_id, $search = "") {
        $sql = "
            SELECT p.patient_id, p.full_name, p.national_id, p.phone, p.gender,
                   pp.policy_number, pp.start_date, pp.end_date, pp.status,
                   ip.plan_name
            FROM patients p
            LEFT JOIN patient_policy pp ON pp.patient_id = p.patient_id AND pp.insurance_id = ?
            LEFT JOIN insurance_plan ip ON ip.id = pp.insurance_plan_id
            WHERE p.insurance_id = ?
              AND p.is_active = 1
        ";
        
        $types = "ii";
        $params = [$insurance_id, $insurance_id];
        
        if ($search !== "") {
            $sql .= " AND (p.full_name LIKE CONCAT('%', ?, '%')
                       OR p.national_id LIKE CONCAT('%', ?, '%')
                       OR p.phone LIKE CONCAT('%', ?, '%'))";
            $types .= "sss";
            $params[] = $search;
            $params[] = $search;
            $params[] = $search;
        }
        
        $sql .= " ORDER BY p.patient_id DESC";
        
        $stmt = $this->conn->prepare($sql);
        $stmt->bind_param($types, ...$params);
        $stmt->execute();
        $result = $stmt->get_result();
        
        $patients = [];
        while ($row = $result->fetch_assoc()) {
            $patients[] = $row;
        }
        $stmt->close();
        return $patients;
    }
    
    /**
     * Get Date of Birth from National ID (Egyptian format)
     */
    public function getDOBFromNationalId() {
        $nid = trim($this->national_id);
        if (!preg_match('/^\d{14}$/', $nid)) return null;
        
        $centuryDigit = (int)$nid[0];
        $yy = (int)substr($nid, 1, 2);
        $mm = (int)substr($nid, 3, 2);
        $dd = (int)substr($nid, 5, 2);
        
        $century = ($centuryDigit === 2) ? 1900 : (($centuryDigit === 3) ? 2000 : null);
        if ($century === null) return null;
        
        $year = $century + $yy;
        if (!checkdate($mm, $dd, $year)) return null;
        
        return sprintf("%04d-%02d-%02d", $year, $mm, $dd);
    }
    
    /**
     * Calculate age from National ID
     */
    public function getAge() {
        $dob = $this->getDOBFromNationalId();
        if (!$dob) return null;
        
        try {
            $d = new DateTime($dob);
            $now = new DateTime();
            return (int)$now->diff($d)->y;
        } catch (Exception $e) {
            return null;
        }
    }
}