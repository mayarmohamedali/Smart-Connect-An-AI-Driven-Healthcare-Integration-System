<?php
// Variables from InsuranceController::policy():
// $insurancePlan (InsurancePlan object), $insurance_name, $insurance_id
// $policies (array from getAllPoliciesForInsurance)
// $completion_status (array: completed, total, percentage)
// $auth, $flash_success, $flash_error
...
<head>
  ...
  <title>Policy Management - <?= e($insurance_name) ?></title>
  ...
  <div class="container-fluid">
    ...
    <div class="collapse navbar-collapse" id="topNavbar">
      <ul class="navbar-nav mr-auto">
        ...
        <li class="nav-item">
          <a class="nav-link" href="<?= BASE_URL ?>/insurance/dashboard#patients">
        ...
        <li class="nav-item dropdown">
          <a class="nav-link dropdown-toggle d-flex align-items-center" href="#" id="userDropdown" role="button"
            ...
            <span class="mr-2 d-none d-lg-inline text-white small">
              <?= e($auth->getSessionData("staff_name") ?? "Insurance Staff") ?>
            </span>
...
<div class="container-fluid mt-4">
  ...
  <div class="d-flex justify-content-between align-items-center mb-4">
    ...
    <div class="text-right">
      <a href="<?= BASE_URL ?>/insurance/createAllPolicies" class="btn btn-primary mb-2">
      ...
      <div class="mb-2">
        ...
        <h4 class="mb-1">
          <?= (int)($completion_status['completed'] ?? 0) ?> / <?= (int)($completion_status['total'] ?? 4) ?> Policies
        </h4>
      </div>
      <div class="completion-bar" style="width: 200px;">
        <div class="progress-fill" style="width: <?= (int)($completion_status['percentage'] ?? 0) ?>%"></div>
      </div>
      <small class="text-muted"><?= (int)($completion_status['percentage'] ?? 0) ?>% Complete</small>
    </div>
  </div>
  ...
  <?php if (($completion_status['completed'] ?? 0) < ($completion_status['total'] ?? 4)): ?>
      ...
      <div class="card policy-card shadow-sm h-100">
        <div class="card-header bg-white py-3 position-relative">
          <div class="policy-status">
            ...
            <?php else: ?>
              <span class="badge badge-warning">
                <i class="fas fa-exclamation-circle mr-1"></i> Default Values
              </span>
            <?php endif; ?>
          </div>
          ...
          <h5 class="mb-1 font-weight-bold text-primary">
            ...
            <?= e($policy['category_name']) ?> - <?= e($policy['customer_type_name']) ?>
          </h5>
          <small class="text-muted">
            Policy ID: <?= $policy['plan_id'] ?? 'Auto-Created' ?>
          </small>
        </div>

        <div class="card-body">
          <?php if ($policy['plan_id']): ?>
            <?php
              $services = $insurancePlan->getServicesByPlanId($policy['plan_id']);
              $enabledServices = array_filter($services, fn($s) => $s['is_enabled']);
            ?>
            <?php if ($enabledServices): ?>
            <div class="mb-3">
              ...
              <div>
                <?php foreach ($enabledServices as $service): ?>
                  <span class="service-badge bg-success text-white">
                    ...
                    (<?= number_format((float)($service['coverage_percent'] ?? 0), 0) ?>%)
                  </span>
                <?php endforeach; ?>
              </div>
            </div>
            ...
            <div class="row text-center mt-3 pt-3 border-top">
              <div class="col-4">
                ...
                <strong class="text-success"><?= count($enabledServices) ?></strong>
              </div>
              <div class="col-4">
                <small class="text-muted d-block">Last Updated</small>
                <strong><?= $policy['updated_at'] ? date('M d, Y', strtotime($policy['updated_at'])) : 'N/A' ?></strong>
              </div>
              <div class="col-4">
            ...
            <div class="text-center py-4">
              ...
              <p class="text-muted mb-0">Policy created with default values.</p>
              <small class="text-muted">Click "Edit Policy" below to customize coverage.</small>
            </div>
            <?php endif; ?>
          <?php else: ?>
            <div class="text-center py-4">
        ...
        <div class="card-footer bg-white border-top">
          <a href="<?= BASE_URL ?>/insurance/editPolicy?category_id=<?= $policy['category_id'] ?>&customer_type_id=<?= $policy['customer_type_id'] ?>"
          ...
          <div class="row">
            ...
            <div class="col-md-6">
              ...
              <ul class="list-unstyled">
                ...
                <li class="mb-2"><i class="fas fa-baby text-info mr-2"></i> Maternity (Optional)</li>
                <li class="mb-2"><i class="fas fa-tooth text-info mr-2"></i> Dental (Optional)</li>
                <li class="mb-2"><i class="fas fa-glasses text-info mr-2"></i> Optical (Optional)</li>
              </ul>
