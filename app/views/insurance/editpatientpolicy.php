...
<head>
  ...
  <title>Edit Patient Policy</title>
...
<div class="container py-4">
  ...
  <div class="card shadow">
    ...
    <div class="card-header text-white" style="background:linear-gradient(135deg,#0ea5e9,#6366f1);">
      <h5 class="mb-0">
        <i class="fas fa-file-signature mr-2"></i>
        Edit Patient Policy —
        <?= ep($patientObj->getFullName()) ?>
      </h5>
      <small>
        National ID: <?= ep($patientObj->getNationalId()) ?> |
        Insurance: <?= ep($insurance_name) ?> |
        Policy #<?= ep($policy['policy_number']) ?>
      </small>
    </div>
    ...
    <div class="card-body">
      ...
      <?php if (!empty($success)): ?>
        <div class="alert alert-success">
          <i class="fas fa-check-circle mr-1"></i> <?= ep($success) ?><br>
      ...
      <?php if (!empty($error)): ?>
        <div class="alert alert-danger"><i class="fas fa-exclamation-circle mr-1"></i> <?= ep($error) ?></div>
      <?php endif; ?>

      <form method="POST" id="editPolicyForm" novalidate>

        <!-- Patient info (read-only) -->
        <h6 class="font-weight-bold mb-3" style="color:#0369a1;">
        ...
        <div class="row">
          <div class="col-md-4 form-group">
            ...
            <input class="form-control bg-light" value="<?= (int)$policy['patient_id'] ?>" readonly tabindex="-1">
          </div>
          <div class="col-md-4 form-group">
            <label>Full Name</label>
            <input class="form-control bg-light" value="<?= ep($patientObj->getFullName()) ?>" readonly tabindex="-1">
          </div>
          <div class="col-md-4 form-group">
            <label>Phone</label>
            <input class="form-control bg-light" value="<?= ep($patientObj->getPhone()) ?>" readonly tabindex="-1">
          </div>
        </div>
          ...
          <div class="col-md-6 form-group">
            ...
            <select class="form-control" id="inp_plan" name="insurance_plan_id">
              ...
              <option value="<?= (int)$pl['id'] ?>"
                <?= ((int)$pl['id'] === (int)$policy['insurance_plan_id']) ? 'selected' : '' ?>>
                <?= Validator::sanitizeInput($pl['plan_name']) ?>
              </option>
              <?php endforeach; ?>
            </select>
        ...
        <div class="row">
          ...
          <div class="col-md-4 form-group">
            ...
            <select class="form-control" id="inp_status" name="status">
              <?php foreach (['active' => 'Active', 'suspended' => 'Suspended', 'expired' => 'Expired'] as $val => $label): ?>
              <option value="<?= $val ?>" <?= ($policy['status'] === $val) ? 'selected' : '' ?>>
                <?= $label ?>
              </option>
              <?php endforeach; ?>
            </select>
        ...
        <div class="d-flex" style="gap:12px;">
          <button class="btn btn-primary flex-grow-1" type="submit" id="btnUpdatePolicy">
            <i class="fas fa-save mr-1"></i> Save Changes
          </button>
          <a href="<?= BASE_URL ?>/insurance/dashboard" class="btn btn-outline-secondary">
            Cancel
          </a>
        </div>
...
(function () {
  ...
  function fail(id,m) { el('inp_'+id).classList.remove('is-valid');   el('inp_'+id).classList.add('is-invalid'); el('err_'+id).textContent=m; }

  function validatePlan() {
    var v = el('inp_plan').value;
    if (!v) { fail('plan','Please select an insurance plan.'); return false; }
    ok('plan'); return true;
  }
  ...
  function validateStartDate() {
    if (!el('inp_start_date').value) { fail('start_date','Start date is required.'); return false; }
    ok('start_date'); return true;
  }
  function validateEndDate() {
    var start = el('inp_start_date').value;
    var end   = el('inp_end_date').value;
    if (end && start && end <= start) { fail('end_date','End date must be after start date.'); return false; }
    el('inp_end_date').classList.remove('is-invalid');
    return true;
  }
  ...
  el('editPolicyForm').addEventListener('submit', function (e) {
    var valid = [validatePlan(), validatePolicyNumber(), validateStartDate(), validateEndDate()].every(Boolean);
