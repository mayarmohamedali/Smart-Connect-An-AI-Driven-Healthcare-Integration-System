...
print("\n" + "="*70)
print(" HOSPITAL EPIDEMIC ALERT SYSTEM - PATIENT LEVEL")
print("="*70)

# Use the best model (make sure this is trained first)
# best_model = rf_epi_model  # Or gb_epi_model, etc.

# Get predictions on TEST set
y_pred_proba = best_model.predict_proba(X_test_epi)[:, 1]  # Probability of epidemic
y_pred = best_model.predict(X_test_epi)

# Create results dataframe from test indices
results_df = df.loc[X_test_epi.index].copy()
results_df['Predicted_Epidemic'] = y_pred
results_df['Epidemic_Probability'] = y_pred_proba
results_df['Actual_Epidemic'] = y_test_epi.values

# Add prediction categories
results_df['Risk_Level'] = pd.cut(
    results_df['Epidemic_Probability'],
    bins=[0, 0.3, 0.6, 1.0],
    labels=['Low Risk', 'Medium Risk', 'High Risk']
)

print(f"\n📊 PREDICTION SUMMARY (Test Set: {len(results_df)} patients):")
print("="*70)
print(f"   Predicted Epidemic Cases: {y_pred.sum()}")
print(f"   Actual Epidemic Cases: {y_test_epi.sum()}")
print(f"   Correctly Identified: {((y_pred == 1) & (y_test_epi == 1)).sum()}")
print(f"\n   Risk Distribution:")
print(f"      High Risk (>60%):   {(results_df['Risk_Level'] == 'High Risk').sum()} patients")
print(f"      Medium Risk (30-60%): {(results_df['Risk_Level'] == 'Medium Risk').sum()} patients")
print(f"      Low Risk (<30%):    {(results_df['Risk_Level'] == 'Low Risk').sum()} patients")

# Filter for HIGH RISK predictions (>=60% probability OR predicted as epidemic)
high_risk = results_df[
    (results_df['Predicted_Epidemic'] == 1) |
    (results_df['Epidemic_Probability'] >= 0.6)
].sort_values('Epidemic_Probability', ascending=False)

print(f"\n🚨 HIGH PRIORITY ALERTS ({len(high_risk)} patients):")
print("="*70)

if len(high_risk) > 0:
    # Group by diagnosis to see patterns
    if 'Diagnosis' in high_risk.columns:
        print("\n📋 Top Predicted Epidemic Diagnoses:")
        top_diagnoses = high_risk['Diagnosis'].value_counts().head(5)
        for diagnosis, count in top_diagnoses.items():
            print(f"   • {diagnosis}: {count} cases")

    print("\n" + "="*70)
    print("🔍 INDIVIDUAL HIGH-RISK CASES (Top 15):")
    print("="*70)

    # Display top 15 high-risk cases
    for i, (idx, row) in enumerate(high_risk.head(15).iterrows(), 1):
        prob = row['Epidemic_Probability'] * 100
        is_correct = "✓" if row['Predicted_Epidemic'] == row['Actual_Epidemic'] else "✗"
        actual_label = "Epidemic" if row['Actual_Epidemic'] == 1 else "Normal"

        print(f"\n  [{i}] Patient Index: {idx} {is_correct}")
        print(f"      Epidemic Risk: {prob:.1f}%")
        print(f"      Prediction: {'EPIDEMIC' if row['Predicted_Epidemic'] == 1 else 'Normal'}")
        ...
        feature_cols = ['Age', 'Gender', 'Admission_Type', 'Blood_Pressure', 'Temperature']
        available_features = [col for col in feature_cols if col in row.index]
        if available_features:
            print(f"      Key Features:")
            for col in available_features[:3]:  # Show top 3 available
                if pd.notna(row[col]):
                    print(f"         • {col}: {row[col]}")
        ...
        if prob >= 80:
            ...
            print(f"         • Isolate patient if infectious disease suspected")
            print(f"         • Alert infection control team")
            print(f"         • Prepare specialized treatment protocols")
        elif prob >= 60:
            print(f"      ⚡ MONITOR CLOSELY")
            print(f"         • Increase observation frequency")
            print(f"         • Prepare outbreak response if pattern emerges")

        print("-"*70)

    # Statistical summary by diagnosis
    print("\n" + "="*70)
    print("📊 EPIDEMIC PATTERN ANALYSIS:")
    print("="*70)

    if 'Diagnosis' in high_risk.columns:
        diagnosis_stats = high_risk.groupby('Diagnosis').agg({
            'Epidemic_Probability': ['count', 'mean', 'max'],
            'Actual_Epidemic': 'sum'
        }).round(3)
        diagnosis_stats.columns = ['Cases', 'Avg_Probability', 'Max_Probability', 'Actual_Epidemics']
        diagnosis_stats = diagnosis_stats.sort_values('Cases', ascending=False).head(10)

        print("\nTop Diagnoses by Predicted Epidemic Cases:")
        print(diagnosis_stats.to_string())
    ...
    true_positives = ((high_risk['Predicted_Epidemic'] == 1) & (high_risk['Actual_Epidemic'] == 1)).sum()
    false_positives = ((high_risk['Predicted_Epidemic'] == 1) & (high_risk['Actual_Epidemic'] == 0)).sum()

    print(f"   True Positives (Correct Epidemic Predictions): {true_positives}")
    print(f"   False Positives (Incorrectly Flagged as Epidemic): {false_positives}")

    if true_positives + false_positives > 0:
        precision = true_positives / (true_positives + false_positives)
        print(f"   Precision for High-Risk Alerts: {precision:.1%}")

else:
    print("\n✅ No high-risk epidemic predictions for the test period")
    print("   System indicates normal disease patterns")

# Summary recommendations
print("\n" + "="*70)
print("💡 HOSPITAL PREPAREDNESS RECOMMENDATIONS:")
print("="*70)

if len(high_risk) > 0:
    total_risk = len(high_risk)

    if total_risk > 50:
        print(f"\n⚠️  ALERT LEVEL: HIGH ({total_risk} high-risk cases)")
        print("   Actions:")
        print("   • Activate epidemic response protocol")
        print("   • Increase staffing in relevant departments")
        print("   • Stock critical medications and supplies")
        print("   • Notify public health authorities")
        print("   • Prepare isolation facilities")
    elif total_risk > 20:
        print(f"\n⚡ ALERT LEVEL: MODERATE ({total_risk} high-risk cases)")
        print("   Actions:")
        print("   • Enhanced surveillance and monitoring")
        print("   • Review supply levels for common epidemic diseases")
        print("   • Brief staff on infection control protocols")
        print("   • Monitor admission trends daily")
    else:
        print(f"\n✓ ALERT LEVEL: LOW ({total_risk} high-risk cases)")
        print("   Actions:")
        print("   • Continue routine monitoring")
        print("   • Maintain standard preparedness levels")
        print("   • Review individual high-risk cases as needed")
else:
    print("\n✅ ALERT LEVEL: MINIMAL")
    print("   Actions:")
    print("   • Maintain routine operations")
    print("   • Continue standard disease surveillance")

print("\n" + "="*70)
print("✅ EPIDEMIC ALERT SYSTEM COMPLETE")
print("="*70)

# Optional: Save high-risk cases to CSV for hospital administration
if len(high_risk) > 0:
    output_cols = ['Epidemic_Probability', 'Predicted_Epidemic', 'Actual_Epidemic', 'Risk_Level']
    if 'Diagnosis' in high_risk.columns:
        output_cols.insert(0, 'Diagnosis')

    available_cols = [col for col in output_cols if col in high_risk.columns]
    high_risk_export = high_risk[available_cols].copy()

    # Uncomment to save:
    # high_risk_export.to_csv('epidemic_alerts_high_risk.csv', index=True)
    # print("\n💾 High-risk alerts saved to: epidemic_alerts_high_risk.csv")
...
# Then use X_train_scaled and X_test_scaled for training
...
# SOLUTION FOR ALREADY PREPROCESSED DATA
# Works with your existing X_train, X_test, y_train, y_test
...
print("="*70)
print("BOOSTING ACCURACY WITH YOUR EXISTING PREPROCESSED DATA")
print("="*70)

# ============================================================================
# STEP 1: CHECK YOUR CURRENT DATA
# ============================================================================
print("\n1️⃣ Checking your existing data...")
...
print(f"Classes in y_train: {sorted(np.unique(y_train))[:15]}")  # Show first 15

# Check if data is scaled
X_train_array = X_train.values if hasattr(X_train, 'values') else X_train
print(f"\nData statistics:")
print(f"   Min: {X_train_array.min():.2f}")
print(f"   Max: {X_train_array.max():.2f}")
print(f"   Mean: {X_train_array.mean():.2f}")
print(f"   Std: {X_train_array.std():.2f}")

is_scaled = X_train_array.std() < 50 and abs(X_train_array.mean()) < 10

if is_scaled:
    print(f"   ✅ Data appears SCALED")
else:
    print(f"   ⚠️  Data appears NOT SCALED - this hurts Logistic Regression!")

# ============================================================================
# STEP 2: SCALE THE DATA (IF NOT ALREADY SCALED)
# ============================================================================
...
if not is_scaled:
    ...
    X_train_scaled = scaler.fit_transform(X_train)
    X_test_scaled = scaler.transform(X_test)
    print(f"   ✅ Scaled! New range: [{X_train_scaled.min():.2f}, {X_train_scaled.max():.2f}]")
else:
    print("   Data already scaled, using as-is")
...
f1_lr = f1_score(y_test, y_pred_test_lr, average='weighted', zero_division=0)
precision_lr = precision_score(y_test, y_pred_test_lr, average='weighted', zero_division=0)
recall_lr = recall_score(y_test, y_pred_test_lr, average='weighted', zero_division=0)
...
f1_gb = f1_score(y_test, y_pred_test_gb, average='weighted', zero_division=0)
precision_gb = precision_score(y_test, y_pred_test_gb, average='weighted', zero_division=0)
recall_gb = recall_score(y_test, y_pred_test_gb, average='weighted', zero_division=0)
...
for i, (model_name, metrics) in enumerate(sorted_results, 1):
    print(f"   {i}. {model_name:25s}: {metrics['test_accuracy']*100:.2f}%")
...
# ============================================================================
print("\n" + "="*70)
print(f"5️⃣ DETAILED REPORT: {best_model_name}")
print("="*70)
...
if best_acc > 0.6:
    print(f"\n✅ SUCCESS! Accuracy improved to {best_acc*100:.1f}%")
    print(f"   Your {best_model_name} is performing well!")
elif best_acc > 0.4:
    print(f"\n⚠️  Moderate improvement to {best_acc*100:.1f}%")
    print(f"   Consider:")
    print(f"   1. Grouping similar diagnoses (if you have 97 classes)")
    print(f"   2. Better feature engineering")
    print(f"   3. Handling class imbalance with SMOTE")
else:
    print(f"\n❌ Still low accuracy: {best_acc*100:.1f}%")
    print(f"   Likely issues:")
    print(f"   1. Too many diagnosis classes (need to group them)")
    print(f"   2. Features not predictive enough")
...
# 75.6 FIXED — ROBUST SOFT VOTING (CALIBRATED) + AUTO WEIGHTS + SAFE CV
# This avoids the "inhomogeneous shape" error and avoids hard-voting nan.
...
print("\n" + "=" * 70)
print("🎯 ROBUST VOTING ENSEMBLE (CALIBRATED SOFT VOTING + AUTO WEIGHTS)")
print("=" * 70)
...
# -----------------------------
# 1) Build CALIBRATED versions of your models
#    - calibrates probabilities => soft voting becomes reliable
#    - also forces consistent predict_proba output shapes
# -----------------------------
cv_cal = 3  # keep small to avoid slow training

xgb_cal = CalibratedClassifierCV(estimator=xgb_risk_model, method="sigmoid", cv=cv_cal)
cat_cal = CalibratedClassifierCV(estimator=catboost_model, method="sigmoid", cv=cv_cal)
rf_cal  = CalibratedClassifierCV(estimator=rf_model,       method="sigmoid", cv=cv_cal)
...
# -----------------------------
# 2) Estimate model strength with CV accuracy (SAFE)
# -----------------------------
cv = StratifiedKFold(n_splits=5, shuffle=True, random_state=42)

print("\n📌 Estimating model strengths with 5-fold CV accuracy...")
means = []
for name, model in estimators:
    scores = cross_val_score(model, X_train, y_train, cv=cv, scoring="accuracy", n_jobs=-1)
    print(f"   {name:>3s} | CV Accuracy: {scores.mean():.4f} ± {scores.std():.4f}")
...
power = 3.0
raw_weights = (means / means.max()) ** power
...
# -----------------------------
# 3) Build SOFT Voting Ensemble ONLY (no hard voting)
# -----------------------------
...
f1w       = f1_score(y_test, y_pred_test, average="weighted", zero_division=0)
precw     = precision_score(y_test, y_pred_test, average="weighted", zero_division=0)
recw      = recall_score(y_test, y_pred_test, average="weighted", zero_division=0)

print("\n" + "=" * 70)
print("📊 ENSEMBLE PERFORMANCE (CALIBRATED SOFT VOTING)")
print("=" * 70)

results_table = pd.DataFrame({
    "Metric": ["Train Accuracy", "Test Accuracy", "F1 (weighted)", "Precision (weighted)", "Recall (weighted)"],
    "Score":  [f"{train_acc:.4f}", f"{test_acc:.4f}", f"{f1w:.4f}", f"{precw:.4f}", f"{recw:.4f}"],
    "Pct":    [f"{train_acc*100:.2f}%", f"{test_acc*100:.2f}%", f"{f1w*100:.2f}%", f"{precw*100:.2f}%", f"{recw*100:.2f}%"]
})
print(results_table.to_string(index=False))

gap = train_acc - test_acc
print(f"\n🔍 Overfit gap (Train-Test): {gap*100:.2f}%")

# -----------------------------
# 5) Confusion Matrix (only if <= 15 classes)
# -----------------------------
cm = confusion_matrix(y_test, y_pred_test)

if "le_target" in globals() and hasattr(le_target, "classes_") and len(le_target.classes_) <= 15:
    plt.figure(figsize=(12, 10))
    sns.heatmap(cm, annot=True, fmt="d", cmap="Blues",
                xticklabels=le_target.classes_,
                yticklabels=le_target.classes_)
    plt.title(f"Confusion Matrix - Calibrated Voting Ensemble\nAcc: {test_acc*100:.1f}% | F1: {f1w*100:.1f}%")
    plt.ylabel("True")
    plt.xlabel("Pred")
    plt.xticks(rotation=45, ha="right")
    plt.tight_layout()
    ...
    print(f"\n⚠️ Confusion matrix plot skipped (too many classes).")
    print(f"   Correct predictions: {np.trace(cm)}/{np.sum(cm)}")

# -----------------------------
# 6) Save to results dict
# -----------------------------
if "results" in globals():
    results["Voting Ensemble (calibrated soft)"] = {
        "model": ensemble,
        ...
        "precision": precw,
        "recall": recw,
        "weights": {name: w for (name, _), w in zip(estimators, weights)}
    }

print("\n✅ Done.")

#76 - ENHANCED MODEL COMPARISON (EXCLUDING WEAK MODELS)

print("\n" + "=" * 70)
print("🏆 ADVANCED MODELS COMPARISON - TOP PERFORMERS ONLY")
print("=" * 70)
...
# Models to exclude (weak performers)
exclude_models = [
    'Logistic Regression',
    'Decision Tree',
    'Naive Bayes',  # If exists
]
...
filtered_results = {
    ...
    if not any(weak in model_name for weak in exclude_models)
}

print(f"\n   Total models trained: {len(results)}")
print(f"   Excluded weak models: {len(results) - len(filtered_results)}")
print(f"   Advanced models remaining: {len(filtered_results)}")

if len(results) - len(filtered_results) > 0:
    excluded_list = [m for m in results.keys() if m not in filtered_results]
    print(f"\n   ❌ Excluded: {', '.join(excluded_list)}")
...
for model_name in filtered_results.keys():
    model_data = filtered_results[model_name]

    train_acc = model_data['train_accuracy'] * 100
    test_acc = model_data['test_accuracy'] * 100
    f1 = model_data['f1_weighted'] * 100
    precision = model_data['precision'] * 100
    recall = model_data['recall'] * 100
    overfit_gap = (model_data['train_accuracy'] - model_data['test_accuracy']) * 100
    ...
    comparison_data.append({
        ...
        '_test_acc': test_acc,
        '_f1': f1,
        '_overfit': overfit_gap
    })

comparison_df = pd.DataFrame(comparison_data)

# Sort by F1-Score (best for medical applications)
comparison_df = comparison_df.sort_values('_f1', ascending=False)

# Display table without internal columns
display_df = comparison_df.drop(['_test_acc', '_f1', '_overfit'], axis=1)
...
# Best by F1-Score (recommended for medical)
best_model_name = max(filtered_results, key=lambda x: filtered_results[x]['f1_weighted'])
best_model_obj = filtered_results[best_model_name]['model']

# Best by Test Accuracy (alternative)
best_acc_name = max(filtered_results, key=lambda x: filtered_results[x]['test_accuracy'])

print(f"\n🥇 CHAMPION (by F1-Score): {best_model_name}")
print(f"   ├─ F1-Score: {filtered_results[best_model_name]['f1_weighted']*100:.2f}% ⭐")
print(f"   ├─ Test Accuracy: {filtered_results[best_model_name]['test_accuracy']*100:.2f}%")
print(f"   ├─ Precision: {filtered_results[best_model_name]['precision']*100:.2f}%")
print(f"   └─ Recall: {filtered_results[best_model_name]['recall']*100:.2f}%")

if best_acc_name != best_model_name:
    print(f"\n🥈 ALTERNATIVE (by Accuracy): {best_acc_name}")
    print(f"   ├─ Test Accuracy: {filtered_results[best_acc_name]['test_accuracy']*100:.2f}%")
    print(f"   └─ F1-Score: {filtered_results[best_acc_name]['f1_weighted']*100:.2f}%")
...
train_acc = filtered_results[best_model_name]['train_accuracy']
test_acc = filtered_results[best_model_name]['test_accuracy']
f1 = filtered_results[best_model_name]['f1_weighted']
precision = filtered_results[best_model_name]['precision']
recall = filtered_results[best_model_name]['recall']
...
if overfit_gap > 0.15:
    status = "❌ SEVERE"
    advice = "Model overfitting badly - use regularization or simpler model"
    color = "red"
elif overfit_gap > 0.10:
    status = "⚠️  MODERATE"
    advice = "Some overfitting - consider regularization"
    color = "orange"
elif overfit_gap > 0.05:
    status = "✓ SLIGHT"
    advice = "Acceptable level - model is learning well"
    color = "yellow"
else:
    status = "✅ EXCELLENT"
    advice = "No overfitting - model generalizes well!"
    color = "green"
...
# Compare to baseline
baseline = max(Counter(y_test).values()) / len(y_test) if 'y_test' in globals() else 0
if baseline > 0:
    ...
    if improvement > 0.20:
        ...
        print(f"      ❌ Barely beats baseline")

# 80% target check
print(f"\n🎯 Target Achievement:")
if test_acc >= 0.80:
    exceed = (test_acc - 0.80) * 100
    print(f"   ✅ 80% TARGET ACHIEVED!")
    print(f"   └─ Exceeded target by: {exceed:.2f}%")
elif test_acc >= 0.75:
    gap = (0.80 - test_acc) * 100
    print(f"   🎯 Close to target!")
    print(f"   └─ Only {gap:.2f}% away from 80%")
else:
    gap = (0.80 - test_acc) * 100
    print(f"   📈 Working towards target")
    print(f"   └─ {gap:.2f}% away from 80%")
...
models_list = list(filtered_results.keys())
test_accs = [filtered_results[m]['test_accuracy'] * 100 for m in models_list]
train_accs = [filtered_results[m]['train_accuracy'] * 100 for m in models_list]
f1_scores = [filtered_results[m]['f1_weighted'] * 100 for m in models_list]
precisions = [filtered_results[m]['precision'] * 100 for m in models_list]
recalls = [filtered_results[m]['recall'] * 100 for m in models_list]

# Figure 1: Test Accuracy vs F1-Score Comparison
print("\n   Creating Figure 1: Test Accuracy vs F1-Score...")
plt.figure(figsize=(16, 6))

x = np.arange(len(models_list))
width = 0.35

plt.bar(x - width/2, test_accs, width, label='Test Accuracy', alpha=0.85,
        color='steelblue', edgecolor='black', linewidth=1.2)
plt.bar(x + width/2, f1_scores, width, label='F1-Score', alpha=0.85,
        color='darkorange', edgecolor='black', linewidth=1.2)

# Add value labels on bars
for i, (acc, f1) in enumerate(zip(test_accs, f1_scores)):
    plt.text(i - width/2, acc + 1, f'{acc:.1f}%', ha='center', va='bottom', fontsize=9, fontweight='bold')
    plt.text(i + width/2, f1 + 1, f'{f1:.1f}%', ha='center', va='bottom', fontsize=9, fontweight='bold')

# Highlight best model
best_idx = models_list.index(best_model_name)
plt.axvline(x=best_idx, color='gold', linestyle='--', linewidth=2, alpha=0.7, label='Best Model')

# Add 80% target line
plt.axhline(y=80, color='red', linestyle='--', linewidth=2, alpha=0.5, label='80% Target')

plt.xlabel('Models', fontsize=13, fontweight='bold')
plt.ylabel('Score (%)', fontsize=13, fontweight='bold')
plt.title('Advanced Models Performance Comparison\n(Weak Models Excluded)',
          fontsize=15, fontweight='bold', pad=20)
plt.xticks(x, models_list, rotation=35, ha='right', fontsize=10)
...
# Figure 2: Comprehensive Metrics Heatmap
print("   Creating Figure 2: Metrics Heatmap...")
...
metrics_matrix = np.array([
    ...
    precisions,
    recalls,
    [abs(train - test) for train, test in zip(train_accs, test_accs)]  # Overfit gap
])
...
sns.heatmap(metrics_matrix,
            ...
            yticklabels=['Test Accuracy', 'F1-Score', 'Precision', 'Recall', 'Overfit Gap'],
...
plt.title('Comprehensive Model Metrics Heatmap\n(Higher is Better, except Overfit Gap)',
...
# Figure 3: Train vs Test Accuracy (Overfitting Check)
print("   Creating Figure 3: Overfitting Analysis...")
plt.figure(figsize=(12, 8))

x_pos = np.arange(len(models_list))
colors = ['green' if abs(tr - te) < 5 else 'orange' if abs(tr - te) < 10 else 'red'
          for tr, te in zip(train_accs, test_accs)]

plt.scatter(train_accs, test_accs, s=300, c=colors, alpha=0.6, edgecolors='black', linewidth=2)

# Add model labels
for i, model in enumerate(models_list):
    plt.annotate(model, (train_accs[i], test_accs[i]),
                fontsize=9, ha='right', va='bottom',
                xytext=(-5, 5), textcoords='offset points')

# Perfect fit line (no overfitting)
max_acc = max(max(train_accs), max(test_accs))
plt.plot([0, max_acc], [0, max_acc], 'k--', linewidth=2, alpha=0.5, label='Perfect Fit (No Overfit)')

# 5% and 10% overfit lines
plt.plot([0, max_acc], [0, max_acc - 5], 'orange', linestyle='--', linewidth=1.5, alpha=0.5, label='5% Overfit')
plt.plot([0, max_acc], [0, max_acc - 10], 'red', linestyle='--', linewidth=1.5, alpha=0.5, label='10% Overfit')

plt.xlabel('Train Accuracy (%)', fontsize=12, fontweight='bold')
plt.ylabel('Test Accuracy (%)', fontsize=12, fontweight='bold')
plt.title('Overfitting Analysis: Train vs Test Accuracy\n(Points closer to diagonal = less overfitting)',
          fontsize=14, fontweight='bold', pad=20)
...
print("   Creating Figure 4: Precision-Recall Balance...")
plt.figure(figsize=(10, 8))

plt.scatter(recalls, precisions, s=300, c=f1_scores, cmap='viridis',
...
print(f"\n✅ Comparison Complete!")
print(f"   ├─ Models compared: {len(filtered_results)}")
print(f"   ├─ Best model: {best_model_name}")
print(f"   ├─ Best F1-Score: {filtered_results[best_model_name]['f1_weighted']*100:.2f}%")
print(f"   └─ Best Test Accuracy: {filtered_results[best_model_name]['test_accuracy']*100:.2f}%")

# Performance tiers
high_performers = [m for m in filtered_results.keys() if filtered_results[m]['test_accuracy'] >= 0.75]
medium_performers = [m for m in filtered_results.keys() if 0.65 <= filtered_results[m]['test_accuracy'] < 0.75]
low_performers = [m for m in filtered_results.keys() if filtered_results[m]['test_accuracy'] < 0.65]

print(f"\n📊 Performance Tiers:")
print(f"   🏆 High (≥75%): {len(high_performers)} models")
if high_performers:
    for m in high_performers:
        print(f"      • {m}: {filtered_results[m]['test_accuracy']*100:.2f}%")

print(f"   🥈 Medium (65-75%): {len(medium_performers)} models")
if medium_performers:
    for m in medium_performers:
        print(f"      • {m}: {filtered_results[m]['test_accuracy']*100:.2f}%")

if low_performers:
    print(f"   🥉 Low (<65%): {len(low_performers)} models")
...
if test_acc < 0.80:
    print(f"   ⚠️  Consider running enhanced ensemble scripts for 80%+ accuracy")
if overfit_gap > 0.10:
    print(f"   ⚠️  Address overfitting with regularization or more data")

print("\n" + "="*70)
print("✅ ADVANCED MODEL COMPARISON COMPLETE!")
print("="*70 + "\n")
